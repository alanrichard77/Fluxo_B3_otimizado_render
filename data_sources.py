...
# ===== Snapshot local (CSV, sem dependências) =====
...
def _get_players_from_carteirafundos() -> pd.DataFrame | None:
    ...
    try:
        ...
        # heurística: primeira com coluna Data e 4+ colunas
        best = None
        for t in tabs:
            cols = list(t.columns)
        ...
        # valores: aceitar números já como float do read_html, senão converter
...
def get_daily_flows_cards():
    """
    Cards de acumulado no ano por player.
    Fontes -> snapshot -> zeros. Não lança exceção.
    """
    ...
    if df is None or df.empty:
        ...
        return {
            ...
            "cards": {
                ...
                "Institucional": zero,
                "Pessoa Física": zero,
                "Inst. Financeira": zero,
                "Outros": zero,
            },
        }

    # Normalização & agregado
    ...
    pivot = (
        df.pivot_table(index="date", columns="player", values="net", aggfunc="sum")
        .fillna(0)
        .cumsum()
    )
